...
export default function Section9IppeoMessage({ paragraphs, final_summary, lang = "ja" }: Section9IppeoMessageProps) {
  return (
    <section>
      <h3 className="text-lg font-bold text-text-dark mb-4 flex items-center gap-2">
        <span className="block w-1 h-6 bg-coral rounded-full"></span>
        {lang === "ko" ? "ARUMI의 한마디" : "ARUMIからの一言"}
      </h3>
      ...
      <div className="mt-6 bg-white rounded-xl p-5 card-shadow border-l-4 border-coral">
        <p className="text-xs font-bold text-coral mb-2">&#x1F4CC; {lang === "ko" ? "최종 정리" : "最終整理"}</p>
