...
8. ご来院予定日 — 相談で言及された日付（なければnull）
9. イッポからの一言 + 最終整理 — 行動誘導型CTA（5つの心理要素を含む）"""
...
"""
    ...
    prompt = f"""以下の情報を元に、9セクションの日本語リポートをJSON形式で作成してください。
...
{{
    ...
    "section8_visit_date": {{
        "date": "YYYY.MM.DD（相談で言及された来院予定日。なければnull）",
        "note": "補足説明（なければnull）"
    }},
...
重要ルール:
- 価格・費用情報は絶対に含めないこと
- 相談で言及されていない医療情報は含めないこと
- PubMed論文の引用（citation）は含めないこと
- 各項目は1〜2文以内で簡潔に
- セクション間の重複・繰り返しを排除すること
- section8_visit_dateのdateとnoteは、相談で言及がなければnullにすること
- 全9セクション必須"""
